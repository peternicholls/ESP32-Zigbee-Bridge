...
static void remove_directory(const char *path) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    int ret =
        snprintf(filepath, sizeof(filepath), "%s/%.255s", path, entry->d_name);
...
static void test_event_filter(void) {
  ...
  os_event_dispatch(0);
  /* Handler should be called by the "all events" subscription from previous
     test, but this test focuses on the filter mechanism */
...
static void test_event_throughput_sc005(void) {
  ...
  /* SC-005: No events dropped under normal operation (≤10 events/second
   * sustained) */
  if (drops > 0) {
...
static void test_quirks_apply_value(void) {
  ...
  os_err_t err = quirks_apply_value("DUMMY", "DUMMY-LIGHT-1", CAP_LIGHT_LEVEL,
                                    &value, &result);
  ASSERT_EQ(err, OS_OK);
  ...
  value.i = 0;
  err = quirks_apply_value("DUMMY", "DUMMY-LIGHT-1", CAP_LIGHT_LEVEL, &value,
                           &result);
  ASSERT_EQ(err, OS_OK);
  ASSERT_TRUE(result.applied);
...
static void test_quirks_action_name(void) {
  ...
  ASSERT_TRUE(
      strcmp(quirks_action_name(QUIRK_ACTION_CLAMP_RANGE), "clamp_range") == 0);
  ASSERT_TRUE(strcmp(quirks_action_name(QUIRK_ACTION_INVERT_BOOLEAN),
                     "invert_boolean") == 0);
  ASSERT_TRUE(strcmp(quirks_action_name(QUIRK_ACTION_SCALE_NUMERIC),
                     "scale_numeric") == 0);
